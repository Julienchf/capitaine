...
import type { HealthEntry, Treatment } from "../lib/types";
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 3, lineHeight: 1.5 }}>
              Génère un lien en lecture seule vers la fiche ci-dessous — sans le budget. Révocable à tout moment.
            </div>
          </div>
        </div>
        <button
          ...
          onClick={() =>
            alert(
              "La génération du lien partageable sera activée avec la synchro cloud (prochaine étape). L'aperçu ci-dessous montre ce que verra la personne.",
            )
          }
        >
          <Icon name="share" size={17} /> Générer le lien
        </button>
      </div>
      ...
      {hasVet && (
        <>
          ...
          <div className="card card-pad">
            ...
            {profile.vetPhone && (
              <a
                href={`tel:${profile.vetPhone.replace(/\s/g, "")}`}
                className="btn primary block"
                style={{ marginTop: profile.vetName ? 12 : 0 }}
              >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 14 }}>
              {profile.vetAddress && (
                <InfoLine icon="hospital">
                  ...
                  {profile.vetMapsUrl && (
                    <a href={profile.vetMapsUrl} target="_blank" rel="noreferrer" style={{ color: "var(--accent-ink)", fontWeight: 500 }}>
                      Voir l'itinéraire
                    </a>
                  )}
                </InfoLine>
              ...
              {profile.vetHours && (
                <InfoLine icon="clock">
                  {profile.vetHours.split("·").map((line, i) => (
              ...
              {profile.vetEmail && (
                <InfoLine icon="mail">
                  <a href={`mailto:${profile.vetEmail}`} style={{ color: "var(--accent-ink)" }}>{profile.vetEmail}</a>
                </InfoLine>
              )}
              {profile.vetWebsite && (
                <InfoLine icon="link">
                  <a href={profile.vetWebsite} target="_blank" rel="noreferrer" style={{ color: "var(--accent-ink)" }}>
                    Site internet
                  </a>
                </InfoLine>
              )}
          ...
          <div className="card" style={{ padding: "0 15px" }}>
            {guideSections.map((s) => (
              <details className="acc" key={s.title}>
                ...
                <div className="acc-body" style={{ whiteSpace: "pre-wrap", color: "var(--text)" }}>
                  {s.content}
                </div>
              </details>
            ))}
        ...
        {care.map((c, i) => (
          <div
            ...
            style={{
              ...
              padding: "9px 0", borderTop: i ? "0.5px solid var(--line)" : "none",
            }}
          >
            <span style={{ fontSize: 14 }}>{CARE_META[c.kind].label}</span>
            <span style={{ fontSize: 13.5, color: "var(--muted)" }}>
              {c.nextDue ? formatDate(c.nextDue) : "à planifier"}
            </span>
          </div>
        ))}
      </div>
...
function TreatmentCard({ t }: { t: Treatment }) {
  ...
  return (
    <div className="card card-pad" style={{ marginBottom: 8 }}>
      ...
      {t.notes && (
        <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 10, fontStyle: "italic" }}>{t.notes}</div>
      )}
    </div>
  );
}
